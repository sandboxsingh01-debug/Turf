  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-4xl px-5 py-16 sm:px-8 sm:py-20">
        ...
        <BookingForm sports={(sports ?? []).map((sport) => ({ id: sport.id, name: sport.name, tagline: sport.description }))} defaultName={user?.user_metadata?.name ?? ''} defaultPhone={user?.user_metadata?.mobile ?? ''} defaultEmail={user?.email ?? ''} />
      </main>
      <Footer />
    </>
  )
